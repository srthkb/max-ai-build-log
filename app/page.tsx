...
export default function Home() {
  ...
  return (
    <main className="site-space overflow-x-clip text-[#f5f9ff]">
      ...
      <section id="projects" className="px-5 py-20 sm:px-6 md:px-12 md:py-36"><div className="mx-auto max-w-7xl"><SectionHeader index="04" title="SELECTED OUTPUT" copy="Things made while learning in public." /><motion.article whileHover={{ transform: "translateY(-5px)" }} className="group mt-10 grid gap-6 border-y border-sky-100/15 py-9 md:grid-cols-[140px_1fr_auto] md:items-start md:py-10"><TextReveal tag="span" text="01 / 01" className="text-[10px] tracking-[.16em] text-sky-300" /><div><TextReveal text="WEB EXPERIENCE" className="text-[10px] tracking-[.16em] text-sky-300" /><TextReveal tag="h2" text="Max / AI Build Log" className="mt-4 text-4xl font-semibold leading-none tracking-[-.08em] sm:text-5xl md:text-7xl" /><TextReveal text="一个持续生长的个人作品档案，也是第一个公开实验。" className="mt-5 text-base leading-relaxed text-sky-50/60" /><Link href="/awakening" className="mt-7 inline-flex min-h-11 items-center rounded-full border border-sky-100/35 px-4 py-3 text-[10px] tracking-[.14em] text-sky-100 transition hover:border-sky-100 hover:bg-sky-100 hover:text-[#06204a]">进入觉醒日志 →</Link></div><span className="grid h-12 w-12 place-items-center rounded-full border border-sky-100/45 text-sky-100 transition group-hover:bg-sky-100 group-hover:text-[#041426]"><ArrowUpRight /></span></motion.article></div></section>

      <section id="about" className="relative overflow-hidden border-t border-sky-100/10 bg-[#07101f] px-5 py-20 sm:px-6 md:px-12 md:py-36"><div className="aurora" /><div className="relative mx-auto grid max-w-7xl gap-10 md:grid-cols-[.7fr_1.3fr]"><TextReveal text="[ 05 — ABOUT MAX ]" className="text-[10px] tracking-[.18em] text-sky-300" /><div><h2 className="text-4xl font-semibold leading-[.86] tracking-[-.08em] sm:text-5xl md:text-8xl"><TextReveal tag="span" text="I learn in public" /><br /><TextReveal tag="span" text="so ideas can become" /> <TextReveal tag="span" text="real." className="text-sky-300" /></h2><TextReveal text="这是我的 AI 实验现场。记录输入，也记录输出；保留过程中的不确定性，更在意真正完成的作品。" className="mt-9 max-w-md text-base leading-relaxed text-sky-50/65 sm:mt-12 sm:text-lg" /><a href="mailto:hello@example.com" className="mt-8 inline-flex min-h-11 items-center border-b border-sky-300 pb-2 text-[11px] tracking-[.12em] text-sky-200">LET&apos;S CONNECT ↗</a></div></div></section>
      <footer className="flex flex-col items-center gap-3 px-5 py-7 text-center text-[10px] tracking-[.12em] text-sky-50/45 sm:flex-row sm:justify-between sm:px-6 sm:text-left md:px-12"><TextReveal tag="span" text="© 2026 MAX" textAlign="center" /><TextReveal tag="span" text="BUILT WITH CURIOSITY + AI" textAlign="center" /><a className="inline-flex min-h-11 items-center" href="#top">BACK TO TOP ↑</a></footer>
    </main>
  );
}

function SectionHeader({ index, title, copy }: { index: string; title: string; copy: string }) { return <div className="flex flex-col justify-between gap-5 border-b border-sky-100/15 pb-6 md:flex-row md:items-end"><div><TextReveal text={`[ ${index} ]`} className="text-[10px] tracking-[.18em] text-sky-300" /><TextReveal tag="h2" text={title} className="mt-3 text-3xl font-semibold tracking-[-.06em] md:text-4xl" /></div><TextReveal text={copy} className="max-w-xs text-base leading-snug text-sky-50/55 md:max-w-44 md:text-right" textAlign="right" /></div>; }

function LogItem({ date, title, copy }: { date: string; title: string; copy: string; delay: number }) { return <li className="grid grid-cols-[66px_1fr] gap-5 py-8"><TextReveal tag="time" text={date} className="text-[10px] tracking-[.12em] text-sky-300" /><div><TextReveal tag="h3" text={title} className="text-lg" /><TextReveal text={copy} className="mt-2 text-sm leading-relaxed text-sky-50/55" /></div></li>; }

function TextReveal({ text, className, tag = "p", textAlign = "left" }: { text: string; className?: string; tag?: "h2" | "h3" | "p" | "span" | "time"; textAlign?: "left" | "right" | "center" }) { return <SplitText tag={tag as "h2" | "h3" | "p" | "span"} text={text} className={className} delay={20} duration={.48} ease="power3.out" from={{ opacity: 0, y: 26 }} to={{ opacity: 1, y: 0 }} threshold={0} rootMargin="0px" textAlign={textAlign} />; }
